  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl">
        <div>
          <h1 className="text-2xl font-black text-uni-text">الإنجازات والنقاط</h1>
          <p className="text-uni-muted text-sm mt-1">تتبع تقدمك واكسب شارات التميز</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card-uni text-center col-span-2 md:col-span-1">
            ...
            <div className="text-xs text-uni-muted mt-1">إجمالي النقاط</div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="card-uni text-center">
            ...
            <div className="text-3xl font-black text-uni-text">المستوى {level}</div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card-uni text-center">
            ...
            <div className="text-xs text-uni-muted mt-1">شارة مكتسبة</div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card-uni text-center">
            ...
            <div className="text-xs text-uni-muted mt-1">شارة متبقية</div>
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="card-uni">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-bold text-uni-text">المستوى {level}: {level_name}</span>
            <span className="text-xs text-uni-muted">{level_progress}/100 نقطة للمستوى التالي</span>
          </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="card-uni">
          <h3 className="font-bold text-uni-text mb-4 flex items-center gap-2">
            <Award className="w-4 h-4 text-uni-gold" /> جميع الشارات
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {badges.map((badge, i) => (
              ...
              >
                ...
                {!!(badge.earned) && (
                  <div className="mt-2">
                    <span className="text-[10px] text-uni-gold border border-uni-gold/30 rounded px-1">مكتسبة</span>
                  </div>
                )}
              </motion.div>
            ))}
